                    ...
                    {underReviewItems.length === 0 ? (
                      ...
                      underReviewItems.map(item => (
                        <div key={item.id} className="bg-gray-50 rounded-lg p-3 border border-gray-100 text-sm">
                          ...
                          <button onClick={() => openModal("confirm_payment", { orderId: item.id })} className="w-full py-1.5 bg-indigo-600 text-white rounded-md text-xs font-bold hover:bg-indigo-700 transition-colors">
                            审核收款
                          </button>
                        </div>
                      ))
                    ...
                    {pendingRefunds.length === 0 ? (
                      ...
                      pendingRefunds.map(refund => (
                        <div key={refund.id} className="bg-gray-50 rounded-lg p-3 border border-gray-100 text-sm">
                          ...
                          <button onClick={() => openModal("seller_refund_audit", { refundId: refund.id })} className="w-full py-1.5 bg-red-600 text-white rounded-md text-xs font-bold hover:bg-red-700 transition-colors">
                            审核处理
                          </button>
                        </div>
                      ))
                      ...
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {item.plan === "按量计费" ? (
                          item.status === "unbilled" ? (
                            <button onClick={() => openModal("generate_invoice", { itemId: item.id })} className="text-sm font-medium flex items-center gap-1.5 justify-center px-3 py-1.5 rounded-lg transition-colors text-indigo-600 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 shadow-sm">
                              <FileText size={14} /> 生成请款单
                            </button>
                          ) : item.status === "under_review" ? (
                            <button onClick={() => openModal("confirm_payment", { itemId: item.id })} className="text-sm font-medium flex items-center gap-1.5 justify-center px-3 py-1.5 rounded-lg transition-colors text-blue-600 bg-blue-50 hover:bg-blue-100 border border-blue-200 shadow-sm">
