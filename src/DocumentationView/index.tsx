...
const DocumentationView: React.FC = () => {
      ...
      <div className="border border-gray-200 rounded-xl overflow-x-auto shadow-sm">
        <table className="w-full text-sm text-left min-w-max">
          <thead className="bg-gray-50 font-bold text-gray-700 border-b border-gray-200">
            <tr>
              {headers.map((h, i) => (
                <th key={i} className="px-6 py-3 whitespace-nowrap bg-gray-50/50">
                  {h}
                </th>
              ))}
            </tr>
            ...
            {rows.map((row, i) => (
              <tr key={i} className="hover:bg-gray-50 transition-colors">
                {row.map((cell, j) => (
                  <td
                    key={j}
                    className="px-6 py-4 text-gray-600 font-mono text-xs md:text-sm leading-relaxed align-top"
                  >
  ...
  const FlowStep = ({ label, desc, type = 'normal' }: any) => {
    ...
    if (type === 'start') {
      ...
      text = 'text-green-800'
      icon = <div className="w-2 h-2 rounded-full bg-green-500 mb-1" />
    }
    if (type === 'decision') {
      bg = 'bg-yellow-50 border-yellow-200'
      text = 'text-yellow-800'
      icon = <div className="w-2 h-2 rotate-45 bg-yellow-500 mb-1" />
    }
    if (type === 'end') {
      bg = 'bg-red-50 border-red-200'
      text = 'text-red-800'
      icon = <div className="w-2 h-2 rounded-sm bg-red-500 mb-1" />
    }
    if (type === 'system') {
      bg = 'bg-blue-50 border-blue-200'
      text = 'text-blue-800'
      icon = <Cpu size={12} className="text-blue-500 mb-1" />
    }
    if (type === 'action') {
      bg = 'bg-gray-50 border-gray-300 border-dashed'
      text = 'text-gray-600'
      icon = <MousePointer size={12} className="text-gray-400 mb-1" />
    }

    return (
      <div
        className={`flex flex-col items-center p-3 rounded-lg border shadow-sm min-w-[120px] max-w-[140px] text-center z-10 transition-transform hover:scale-105 ${bg}`}
      >
        {icon}
  ...
  const FlowArrow = ({ label }: { label?: string }) => (
    <div className="flex-1 h-px bg-gray-300 relative mx-2 min-w-[30px] flex items-center justify-center">
      {label && (
        <span className="absolute -top-3 text-[9px] text-gray-500 bg-white px-1 whitespace-nowrap border border-gray-100 rounded">
          {label}
        </span>
      )}
  ...
  const renderOverview = () => (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <p className="text-blue-100/70 text-sm leading-relaxed text-justify">
                我们致力于构建一个打破 OT（生产技术）与
                IT（信息技术）边界的数字化操作系统。通过标准化的工业智能体网络，将顶尖专家的隐性知识转化为可计算、可分发的算法资产，助力全球钢企实现冷轧产线的零缺陷、全自动与可持续制造。
              </p>
            </div>
            ...
            <div className="lg:col-span-3 grid grid-cols-1 md:grid-cols-2 gap-4">
              {[
                {
                  title: '连接 (Connect)',
                  desc: '消除工序壁垒，实现从炼铁到冷轧全流程 100% 数字化感知。',
                  icon: Share2,
                  color: 'text-blue-400',
                },
                {
                  title: '赋能 (Empower)',
                  desc: '使中小钢企以 1/10 的成本获取世界级工艺模型。',
                  icon: Zap,
                  color: 'text-yellow-400',
                },
                {
                  title: '标准化 (Standard)',
                  desc: '定义工业知识资产化的 I/O 协议，实现知识即服务 (KaaS)。',
                  icon: ShieldCheck,
                  color: 'text-green-400',
                },
                {
                  title: '生态 (Ecosystem)',
                  desc: '汇聚 10,000+ 工业开发者，打造千亿级规模的智能资产市场。',
                  icon: Trophy,
                ...
                <div
                  key={i}
                  className="bg-white/5 hover:bg-white/10 p-5 rounded-2xl border border-white/10 transition-all group backdrop-blur-sm"
                >
      ...
      <div className="mb-12">
        <h3 className="text-lg font-bold text-gray-900 mb-6 border-l-4 border-indigo-600 pl-3">
          1.2 核心用户画像 (Core Personas)
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm relative overflow-hidden">
            ...
            <ul className="space-y-3 text-sm text-gray-600">
              <li className="flex gap-2">
                <strong className="text-gray-900 min-w-[40px]">痛点:</strong>{' '}
                产线质量波动大，缺乏高级工艺模型，自研成本高。
              </li>
              <li className="flex gap-2">
                <strong className="text-gray-900 min-w-[40px]">目标:</strong>{' '}
                快速引入成熟算法解决板形、厚度等具体质量问题，降低废品率。
              </li>
              <li className="flex gap-2">
                <strong className="text-gray-900 min-w-[40px]">场景:</strong>{' '}
                生产监控室、质量管理部、IT中心。
              </li>
            </ul>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm relative overflow-hidden">
            ...
            <ul className="space-y-3 text-sm text-gray-600">
              <li className="flex gap-2">
                <strong className="text-gray-900 min-w-[40px]">痛点:</strong>{' '}
                优质算法缺乏商业化落地渠道，知识产权保护难，交付部署繁琐。
              </li>
              <li className="flex gap-2">
                <strong className="text-gray-900 min-w-[40px]">目标:</strong>{' '}
                将沉淀的工业机理模型标准化封装，实现知识变现与规模化分发。
              </li>
              <li className="flex gap-2">
                <strong className="text-gray-900 min-w-[40px]">场景:</strong> 实验室、软件开发中心。
              </li>
            </ul>
          </div>
        </div>
  ...
  const renderArchitecture = () => (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="mb-12">
        <h3 className="text-lg font-bold text-gray-900 mb-6 border-l-4 border-blue-600 pl-3">
        ...
        <div className="bg-slate-50 p-8 rounded-2xl border border-slate-200">
          <div className="flex flex-col gap-4 max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="bg-white p-3 rounded-lg border border-gray-200 text-center shadow-sm text-sm font-bold text-gray-700">
                Web 端 (React/TS)
              </div>
              <div className="bg-white p-3 rounded-lg border border-gray-200 text-center shadow-sm text-sm font-bold text-gray-700">
                Mobile 端 (H5/App)
            ...
            <div className="grid grid-cols-4 gap-4 p-4 bg-white rounded-xl border border-dashed border-gray-300">
              <div className="text-center text-xs font-bold text-gray-400 col-span-4 mb-2 uppercase tracking-widest">
                Application Layer
              </div>
              <div className="bg-blue-50 p-4 rounded-lg text-center text-blue-800 font-bold text-sm">
                市场前台
                <br />
                <span className="text-xs font-normal opacity-70">Marketplace</span>
              </div>
              <div className="bg-green-50 p-4 rounded-lg text-center text-green-800 font-bold text-sm">
                买家工作台
                <br />
                <span className="text-xs font-normal opacity-70">Buyer Console</span>
              </div>
              <div className="bg-purple-50 p-4 rounded-lg text-center text-purple-800 font-bold text-sm">
                卖家控制台
                <br />
                <span className="text-xs font-normal opacity-70">Seller Console</span>
              </div>
              <div className="bg-gray-100 p-4 rounded-lg text-center text-gray-800 font-bold text-sm">
                运营后台
                <br />
                <span className="text-xs font-normal opacity-70">Admin Panel</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-5 gap-2">
              {[
                ...
                <div
                  key={svc}
                  className="bg-white border border-gray-200 p-2 rounded text-center text-xs font-medium text-gray-600 shadow-sm"
                >
                  {svc}
                </div>
              ))}
            ...
            <div className="bg-slate-800 p-4 rounded-xl text-center text-white text-xs space-y-2">
              <div className="font-bold opacity-50 uppercase tracking-widest mb-2">
      ...
      <div>
        <h3 className="text-lg font-bold text-gray-900 mb-6 border-l-4 border-purple-600 pl-3">
          2.2 产品功能结构图 (Functional Structure)
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="border border-gray-200 rounded-xl overflow-hidden">
            ...
            <div className="p-4 space-y-2 text-sm text-gray-600">
              <div className="flex gap-2">
                <div className="w-1.5 h-1.5 bg-blue-400 rounded-full mt-1.5"></div> 全局搜索与筛选
              </div>
              <div className="flex gap-2">
                <div className="w-1.5 h-1.5 bg-blue-400 rounded-full mt-1.5"></div> 智能体分类导航
              </div>
              <div className="flex gap-2">
                <div className="w-1.5 h-1.5 bg-blue-400 rounded-full mt-1.5"></div> 商品详情页
                (Demo/Video)
              </div>
              <div className="flex gap-2">
                <div className="w-1.5 h-1.5 bg-blue-400 rounded-full mt-1.5"></div> 需求广场
              </div>
              <div className="flex gap-2">
                <div className="w-1.5 h-1.5 bg-blue-400 rounded-full mt-1.5"></div> 资源包选购
              </div>
            </div>
          </div>
          {/* Buyer */}
          <div className="border border-gray-200 rounded-xl overflow-hidden">
            <div className="bg-green-50 px-4 py-3 border-b border-green-100 font-bold text-green-800">
              采购工作台 (Buyer)
            </div>
            <div className="p-4 space-y-2 text-sm text-gray-600">
              <div className="flex gap-2">
                <div className="w-1.5 h-1.5 bg-green-400 rounded-full mt-1.5"></div> 仪表盘
                (Dashboard)
              </div>
              <div className="flex gap-2">
                <div className="w-1.5 h-1.5 bg-green-400 rounded-full mt-1.5"></div> 订单管理 & 续费
              </div>
              <div className="flex gap-2">
                <div className="w-1.5 h-1.5 bg-green-400 rounded-full mt-1.5"></div> 资源用量监控
              </div>
              <div className="flex gap-2">
                <div className="w-1.5 h-1.5 bg-green-400 rounded-full mt-1.5"></div> 账单与发票
              </div>
              <div className="flex gap-2">
                <div className="w-1.5 h-1.5 bg-green-400 rounded-full mt-1.5"></div> 工单支持
              </div>
            </div>
          </div>
          {/* Seller */}
          <div className="border border-gray-200 rounded-xl overflow-hidden">
            ...
            <div className="p-4 space-y-2 text-sm text-gray-600">
              <div className="flex gap-2">
                <div className="w-1.5 h-1.5 bg-purple-400 rounded-full mt-1.5"></div> 收益总览
              </div>
              <div className="flex gap-2">
                <div className="w-1.5 h-1.5 bg-purple-400 rounded-full mt-1.5"></div> 资产发布向导
              </div>
              <div className="flex gap-2">
                <div className="w-1.5 h-1.5 bg-purple-400 rounded-full mt-1.5"></div> 版本管理
              </div>
              <div className="flex gap-2">
                <div className="w-1.5 h-1.5 bg-purple-400 rounded-full mt-1.5"></div> 运营数据分析
              </div>
              <div className="flex gap-2">
                <div className="w-1.5 h-1.5 bg-purple-400 rounded-full mt-1.5"></div> API Key 管理
              </div>
            </div>
          </div>
        </div>
  ...
  const renderDetailedFlows = () => (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="mb-12">
        <h3 className="text-lg font-bold text-gray-900 mb-6 border-l-4 border-indigo-600 pl-3">
          3.1 资产发布与上架流程 (Asset Publishing)
        </h3>
        <div className="bg-white p-8 rounded-2xl border border-gray-200 overflow-x-auto shadow-sm">
          ...
          <div className="mt-6 pt-4 border-t border-gray-100 flex gap-4 text-xs text-gray-500">
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-red-500"></div> 异常流：安全扫描失败 →
              驳回修改 → 重新提交
            </div>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-yellow-500"></div> 异常流：人工审核拒绝 →
              发送通知 → 开发者修改
            </div>
          </div>
        </div>
      ...
      <div className="mb-12">
        <h3 className="text-lg font-bold text-gray-900 mb-6 border-l-4 border-blue-600 pl-3">
          3.2 购买与履约流程 (Purchase & Fulfillment)
        </h3>
        <div className="bg-white p-8 rounded-2xl border border-gray-200 overflow-x-auto shadow-sm">
          <div className="flex items-center gap-2 min-w-[1000px]">
      ...
      <div className="mb-12">
        <h3 className="text-lg font-bold text-gray-900 mb-6 border-l-4 border-green-600 pl-3">
          3.3 资源包挂载流程 (Resource Pack Mounting)
        </h3>
      ...
      {/* 3.4 Support Flow */}
      <div className="mb-12">
        <h3 className="text-lg font-bold text-gray-900 mb-6 border-l-4 border-orange-600 pl-3">
          3.4 售后与退款流程 (Support & Refund)
        </h3>
        <div className="bg-white p-8 rounded-2xl border border-gray-200 overflow-x-auto shadow-sm">
          <div className="flex items-center gap-2 min-w-[1000px]">
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <ModuleSection title="4.1 全局导航与鉴权 (Sidebar & Auth)" id="feat-nav">
        <SpecTable
          ...
          rows={[
            [
              '<strong>角色切换 (Console Switch)</strong>',
              'Admin/Developer',
              '允许在“买家工作台”与“开发者后台”之间无缝切换。',
              '点击 Sidebar 底部账户卡片 -> 选择“切换身份”',
            ],
            [
              '<strong>身份模拟 (Identity Mock)</strong>',
              'Admin Only',
              '用于演示目的，快速切换不同用户身份（如张采购、王工）。',
              'DevTools 浮窗或账户菜单中的隐藏入口',
            ],
            [
              '<strong>发布入口 (Publish Entry)</strong>',
              'Developer/Admin',
              '仅在“开发者模式”下可见。',
              'Sidebar 中部显著的“发布智能体”按钮',
            ],
            [
              '<strong>消息徽标 (Badge)</strong>',
              'Any Role',
              '实时显示未读消息数，包含系统通知与私信。',
              'WebSocket 推送更新 Sidebar 菜单上的红点',
            ],
          ]}
        />
      </ModuleSection>
      ...
      <ModuleSection title="4.2 市场探索 (Discovery Marketplace)" id="feat-discovery">
        <SpecTable
          ...
          rows={[
            [
              '<strong>全局搜索 (Global Search)</strong>',
              '支持模糊匹配 Title, Tags, Provider。防抖 (Debounce) 300ms。',
              'ElasticSearch / DB',
              '相关度降序 > 销量降序 > 评分降序',
            ],
            [
              '<strong>类目筛选 (Category Filter)</strong>',
              '点击 Tag 切换，支持多选（如 Method + Steel）。',
              'Metadata Tags',
              '精确匹配',
            ],
            [
              '<strong>智能体卡片 (Agent Card)</strong>',
              '展示封面、评分、销量。Hover 时显示“快速预览”按钮，点击进入详情页。',
              'Assets Table',
              'N/A',
            ],
            [
              '<strong>榜单推荐 (Ranking)</strong>',
              '基于 CTR 和 Conversion Rate 的加权算法推荐。',
              'User Behavior Log',
              'Top 3 高亮展示',
            ],
          ]}
      ...
      <ModuleSection title="4.3 智能体详情页 (Product Detail)" id="feat-detail">
        <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 mb-6 text-sm text-blue-800">
          <strong>核心设计：</strong>{' '}
          详情页不仅仅是信息展示，更是“试用”与“交易”的决策中心。集成了实时预览 (Live
          Demo)、视频介绍与动态定价计算器。
        </div>
        <SpecTable
          ...
          rows={[
            [
              '<strong>多维演示 (Demo Container)</strong>',
              '支持切换：1. 界面截图 2. 宣传视频 3. 实时交互组件 (Live Component)。',
              'Tab 切换触发不同组件渲染；Live 模式下模拟真实数据流。',
            ],
            [
              '<strong>模块化详情 (Modular Detail)</strong>',
              '支持 Hero, Features, Comparison, Architecture 等多种布局模块。',
              '根据 JSON 配置动态渲染不同类型的 React 组件。',
            ],
            [
              '<strong>动态定价 (Pricing Calculator)</strong>',
              '根据用户选择的模式（订阅/买断）及版本，实时计算总价。',
              '单选框切换触发价格重算；“立即获取”按钮触发购买弹窗。',
            ],
            [
              '<strong>合规披露 (Compliance)</strong>',
              '强制展示数据脱敏声明与知识产权归属。',
              '静态展示，鼠标 Hover 显示详细解释。',
            ],
          ]}
        />
      </ModuleSection>
        ...
        <div className="grid grid-cols-2 gap-6 mb-6">
          ...
          <div className="border border-gray-200 p-4 rounded-lg">
            ...
            <ul className="list-disc pl-4 text-xs text-gray-600 space-y-1">
              <li>配置订阅制/买断制参数</li>
              <li>
                <strong>收入模拟器：</strong>拖动滑块实时测算开发者净收益
              </li>
      ...
      <ModuleSection title="4.5 资源包中心 (Resource Packs)" id="feat-resource">
        <SpecTable
          ...
          rows={[
            [
              '<strong>Token 包 (算力)</strong>',
              'k Tokens',
              '大语言模型对话、逻辑推理、代码生成。',
              '选择规格 -> 选择有效期 -> 加入清单 -> 结算',
            ],
            [
              '<strong>Storage 包 (存储)</strong>',
              'GB',
              '向量数据库扩容、历史生产数据归档。',
              '同上，支持多选不同规格合并支付',
            ],
            [
              '<strong>API Calls 包 (调用)</strong>',
              '次 (Calls)',
              '传统判别式模型的 API 接口调用。',
              '按次扣除，失败不计费',
            ],
      ...
      <ModuleSection title="4.6 采购工作台 (Buyer Console)" id="feat-buyer">
        ...
        <SpecTable
          ...
          rows={[
            ...
            [
              '<strong>资源概览图</strong>',
              '可视化展示 Token/Storage 每日消耗趋势。',
              'ECharts 柱状/折线混合图，Hover 显示具体数值',
            ],
          ]}
        />
        ...
        <SpecTable
          ...
          rows={[
            [
              '<strong>订单列表</strong>',
              '支持按状态 (Active/Expired) 筛选。',
              'Tab 切换状态，搜索框过滤订单号/产品名',
            ],
            [
              '<strong>订单详情</strong>',
              '查看详细配置、历史记录、续费入口。',
      ...
      <ModuleSection title="4.7 开发者控制台 (Seller Console)" id="feat-seller">
        ...
        <SpecTable
          ...
          rows={[
            [
              '<strong>资产列表</strong>',
              '展示所有发布资产及其状态（上架/审核/草稿）。',
              'Tab 切换状态，列表项包含版本信息、健康度',
            ],
            [
              '<strong>版本管理</strong>',
              '发布新版本、回滚旧版本、弃用版本。',
              'Modal 弹窗管理版本历史，支持“发布新版本”入口',
            ],
            [
              '<strong>资产编辑</strong>',
              '修改资产元数据（描述、标签）。',
        ...
        <SpecTable
          ...
          rows={[
            ...
            [
              '<strong>提现申请</strong>',
              '发起资金提现请求。',
              '点击“申请提现”按钮弹出金额输入 Modal',
            ],
            [
              '<strong>退款审核</strong>',
              '审核买家的退款申请。',
              '列表展示待办事项，点击“处理”弹出审核详情与操作按钮',
            ],
          ]}
        />
      </ModuleSection>
      ...
      <ModuleSection title="4.8 系统设置 (Settings)" id="feat-settings">
        <SpecTable
          ...
          rows={[
            [
              '<strong>基本资料 (Profile)</strong>',
              '头像、昵称、企业认证信息修改。',
              '表单编辑，上传图片',
            ],
            [
              '<strong>安全设置 (Security)</strong>',
              '登录密码修改、MFA 多因素认证绑定。',
              'Toggle 开关控制 MFA，修改密码需原密码验证',
            ],
            [
              '<strong>API 密钥 (Keys)</strong>',
              '创建、删除 API Key。',
              '列表展示 Key（脱敏），点击“新建”弹出配置 Modal，生成后仅显示一次',
            ],
        ...
        <div className="bg-yellow-50 border border-yellow-100 rounded-xl p-5 mb-6">
          ...
          <p className="text-sm text-yellow-700 leading-relaxed mb-3">
            API Key 是开发者在第三方应用（如 ERP、MES、L2 控制系统）中调用平台智能体服务的
            <strong>唯一身份凭证</strong>。
          </p>
          <ul className="text-xs text-yellow-700 list-disc pl-4 space-y-1">
            <li>
              <strong>系统集成必选：</strong> 工业现场系统必须通过 API Key 进行鉴权。
            </li>
            <li>
              <strong>最小权限原则：</strong> 建议为不同项目生成独立 Key。
            </li>
            <li>
              <strong>环境隔离：</strong> 生产环境与测试环境 Key 应严格区分。
            </li>
          </ul>
        </div>
      </ModuleSection>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        <div className="bg-white p-8 rounded-2xl border border-gray-200 shadow-sm">
          <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
            <Scale size={18} /> 定价公式 (Pricing Formula)
          </h3>
          ...
          <ul className="text-sm text-gray-600 space-y-3">
            <li className="flex gap-2">
              ...
              <div>
                <strong>Base_Fee:</strong> 订阅周期内的固定费用（如 ¥5,800/月），包含基础 SLA 和
                基础 Quota。
              </div>
            </li>
            <li className="flex gap-2">
              <span className="w-1.5 h-1.5 bg-blue-500 rounded-full mt-1.5"></span>{' '}
              <div>
                <strong>Usage_i:</strong> 超出基础 Quota 后的增量使用量 (如 Token 数)。
              </div>
            </li>
            <li className="flex gap-2">
              <span className="w-1.5 h-1.5 bg-blue-500 rounded-full mt-1.5"></span>{' '}
              <div>
                <strong>Unit_Rate_i:</strong> 阶梯定价费率。用量越大，费率越低。
              </div>
            </li>
          </ul>
        </div>
        ...
        <div className="bg-white p-8 rounded-2xl border border-gray-200 shadow-sm">
          <h3 className="font-bold text-gray-900 mb-4 flex items-center gap-2">
            <PieChart size={18} /> 分成与结算 (Settlement)
          </h3>
          <div className="flex items-center gap-1 mb-4 w-full h-10 rounded-lg overflow-hidden">
            <div className="flex-1 bg-blue-100 flex items-center justify-center text-xs font-bold text-blue-800 border-r border-white">
              开发者 85%
            </div>
            <div className="w-[15%] bg-gray-200 flex items-center justify-center text-xs font-bold text-gray-600">
              平台 15%
            </div>
          </div>
          <p className="text-xs text-gray-500 leading-relaxed">
            平台收取 15% 技术服务费，用于覆盖支付通道费、服务器带宽及运营成本。 结算周期为{' '}
            <strong>T+7 (月结)</strong>。即本月产生的收益，将在次月 7 日汇入开发者对公账户。
          </p>
        </div>
      </div>
        ...
        <SpecTable
          ...
          rows={[
            [
              ...
              'High',
              '例如：独家的高炉炉腹温度场模型，定价可高于市场均值 200%。',
            ],
            [
              '<strong>算力消耗 (Compute)</strong>',
              'Medium',
              '包含大模型的 Token 消耗、GPU 渲染时长。直接计入 Unit Rate。',
            ],
            [
              '<strong>数据价值 (Data Value)</strong>',
              'High',
              '涉及行业对标数据（Analysis Agent），数据本身即资产。',
            ],
            [
              '<strong>部署方式 (Deployment)</strong>',
              'High',
              '私有化部署通常为订阅年费的 3-5 倍 (一次性买断)。',
            ],
          ]}
        />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
        <div className="bg-white p-8 rounded-2xl border border-gray-200 shadow-sm">
          <h3 className="font-bold text-gray-900 mb-6 flex items-center gap-2">
            <Zap size={20} className="text-yellow-500" /> 性能指标 (Performance)
          </h3>
        ...
        <div className="bg-white p-8 rounded-2xl border border-gray-200 shadow-sm">
          <h3 className="font-bold text-gray-900 mb-6 flex items-center gap-2">
            <ShieldCheck size={20} className="text-green-500" /> 安全与合规 (Security)
          </h3>
          <ul className="space-y-4 text-sm text-gray-600">
            <li className="flex gap-3 items-center">
              <Lock size={16} className="text-gray-400" /> <strong>传输加密：</strong> 全链路 TLS
              1.3 加密。
            </li>
            <li className="flex gap-3 items-center">
              <Eye size={16} className="text-gray-400" /> <strong>数据隐私：</strong>{' '}
              敏感生产数据（如配方）在存储层进行 AES-256 加密。
            </li>
            <li className="flex gap-3 items-center">
              <Server size={16} className="text-gray-400" /> <strong>隔离性：</strong>{' '}
              智能体运行在独立的沙箱环境（Sandbox）中。
            </li>
          </ul>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
        <div className="bg-gray-50 px-8 py-5 border-b border-gray-200">
          <h3 className="font-bold text-gray-800 text-lg">
            6.2 关键数据埋点与指标 (Analytics & Tracking)
          </h3>
          <p className="text-sm text-gray-500 mt-1">
            为了持续优化平台，我们构建了统一的监控体系，涵盖顶层业务指标 (Metrics) 与底层行为埋点
            (Events)。
          </p>
        </div>
        <div className="p-8">
          <SpecTable
            headers={[
              ...
              '监控项 (Item / Key)',
              '定义 / 触发条件 (Definition)',
              '数据源 / 参数 (Source / Params)',
              '业务目标 (Goal)',
            ]}
            rows={[
              // Business Metrics
              [
                '<span class="bg-blue-100 text-blue-700 px-2 py-1 rounded text-xs font-bold border border-blue-200">核心指标</span>',
                '<strong>交易漏斗</strong>',
                '详情页UV → 试用转化率 → 购买转化率',
                '页面PV，试用按钮Click，支付成功回调',
                '评估智能体吸引力与定价合理性',
              ],
              [
                '<span class="bg-blue-100 text-blue-700 px-2 py-1 rounded text-xs font-bold border border-blue-200">核心指标</span>',
              ...
              // Domain Metrics
              [
                '<span class="bg-green-100 text-green-700 px-2 py-1 rounded text-xs font-bold border border-green-200">工业指标</span>',
                '<strong>缺陷拦截率</strong>',
                'AI识别缺陷且被人工复核确认为真的比例',
                '质检反馈日志 (Feedback Loop)',
                '验证冷轧表面检测模型(ASI)的实战效果',
              ],
              [
                '<span class="bg-green-100 text-green-700 px-2 py-1 rounded text-xs font-bold border border-green-200">工业指标</span>',
                '<strong>控制时延 (Loop Latency)</strong>',
                '从采集轧机信号到下发调整指令的时间差',
                '边缘计算节点 (Edge Node) 日志',
                '确保板形自动控制 (AFC) 满足毫秒级响应要求',
              ],

              // Event Tracking
              [
                '<span class="bg-purple-100 text-purple-700 px-2 py-1 rounded text-xs font-bold border border-purple-200">埋点事件</span>',
                '<code class="bg-gray-100 px-1 rounded text-purple-600">view_product</code>',
                '用户进入详情页',
                '<code class="bg-gray-100 px-1 rounded text-gray-600">{asset_id, source_ref}</code>',
                '计算转化漏斗 (CTR)',
              ],
              [
                '<span class="bg-purple-100 text-purple-700 px-2 py-1 rounded text-xs font-bold border border-purple-200">埋点事件</span>',
                '<code class="bg-gray-100 px-1 rounded text-purple-600">click_purchase</code>',
                '点击订阅或购买按钮',
                '<code class="bg-gray-100 px-1 rounded text-gray-600">{pricing_mode, user_role}</code>',
                '评估购买意愿',
              ],
              [
                '<span class="bg-purple-100 text-purple-700 px-2 py-1 rounded text-xs font-bold border border-purple-200">埋点事件</span>',
                '<code class="bg-gray-100 px-1 rounded text-purple-600">api_invoke_success</code>',
                '智能体API调用成功',
                '<code class="bg-gray-100 px-1 rounded text-gray-600">{latency, token_usage}</code>',
                '计费与性能监控',
              ],
              [
                '<span class="bg-purple-100 text-purple-700 px-2 py-1 rounded text-xs font-bold border border-purple-200">埋点事件</span>',
                '<code class="bg-gray-100 px-1 rounded text-purple-600">api_invoke_error</code>',
                '调用返回4XX/5XX状态码',
                '<code class="bg-gray-100 px-1 rounded text-gray-600">{error_code, endpoint}</code>',
                '健康度诊断',
              ],
              [
                '<span class="bg-purple-100 text-purple-700 px-2 py-1 rounded text-xs font-bold border border-purple-200">埋点事件</span>',
                '<code class="bg-gray-100 px-1 rounded text-purple-600">resource_pack_mount</code>',
                '资源包挂载操作',
                '<code class="bg-gray-100 px-1 rounded text-gray-600">{resource_id, instance_id}</code>',
                '资源利用率分析',
              ],
              [
                '<span class="bg-purple-100 text-purple-700 px-2 py-1 rounded text-xs font-bold border border-purple-200">埋点事件</span>',
                '<code class="bg-gray-100 px-1 rounded text-purple-600">model_inference</code>',
                '冷轧机理模型完成一次推演',
                '<code class="bg-gray-100 px-1 rounded text-gray-600">{input_coil_id, output_flatness}</code>',
                '监控模型在不同钢种下的表现',
              ],
            ]}
          />
        ...
        {[
          {
            ...
            time: '2025 Q1',
            title: '基础交易闭环',
            desc: '上线市场前台、买家/卖家控制台核心功能，支持 API Key 鉴权与基础计费。',
            status: 'done',
          },
          {
            ...
            time: '2025 Q2',
            title: '开发者生态增强',
            desc: '开放 OpenAPI，提供 SDK，支持更复杂的智能体类型（如可视化组件）。引入社区评价体系。',
            status: 'current',
          },
          {
            ...
            time: '2025 Q3',
            title: 'AI 辅助与 RAG',
            desc: '集成 Copilot 助手，支持自然语言检索市场资产。上线基于 RAG 的智能文档问答。',
            status: 'planned',
          },
          {
            phase: 'Phase 4: Interconnection',
            time: '2025 Q4',
            title: '工业互联深度集成',
            desc: '支持与主流工业互联网平台（如 MindSphere, Predix）的直接互通，实现资产的一键部署。',
            status: 'planned',
          },
        ].map((item, idx) => (
          <div key={idx} className="relative">
            <div
              className={`absolute -left-[42px] top-0 w-5 h-5 rounded-full border-4 border-white ${
                item.status === 'done'
                  ? 'bg-green-500'
                  : item.status === 'current'
                    ? 'bg-blue-600'
            ...
            <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start mb-2">
                ...
                {item.status === 'current' && (
                  <span className="text-xs text-blue-600 font-bold animate-pulse">进行中...</span>
                )}
              </div>
  ...
  const renderContent = () => {
    switch (activeSection) {
      ...
      case 'flows':
        return renderDetailedFlows()
      case 'features':
        return renderDetailedFeatures()
      case 'economics':
  ...
  return (
    <div className="flex h-full bg-white font-sans text-gray-900">
        ...
        <div className="px-4 py-4 mt-auto">
          ...
          {activeSection === 'features' && (
            <div className="space-y-1 border-l-2 border-gray-200 ml-2 pl-2">
              <a href="#feat-nav" className="block text-xs text-gray-500 hover:text-blue-600 py-1">
                4.1 导航与鉴权
              </a>
              <a
                href="#feat-discovery"
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar bg-white scroll-smooth">
        <div className="max-w-5xl mx-auto p-12">{renderContent()}</div>
      </div>
    </div>
  )
}
