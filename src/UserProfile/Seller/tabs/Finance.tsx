...
const SellerFinance: React.FC = () => {
  ...
  const setSellerFinanceSubTab = (tab: string) => {
    ...
    <div className="space-y-8 animate-in fade-in pb-10">
      ...
      <div className="flex gap-8 border-b border-gray-100">
        {['overview', 'monitoring', 'transactions'].map((tab) => (
          <button
            key={tab}
            onClick={() => setSellerFinanceSubTab(tab)}
            className={`pb-4 text-sm font-bold relative transition-colors ${
              sellerFinanceSubTab === tab ? 'text-blue-600' : 'text-gray-500 hover:text-gray-800'
            }`}
          >
            {tab === 'overview'
              ? '综合看板'
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            <div className="bg-blue-600 text-white p-8 rounded-2xl shadow-lg shadow-blue-200 lg:w-1/3 relative overflow-hidden">
              <div className="relative z-10">
                <div className="text-blue-200 text-xs font-bold uppercase tracking-wider mb-2">
                  本月已收款 (RECEIVED)
                </div>
                    ...
                    {underReviewItems.length === 0 ? (
                      ...
                      underReviewItems.map((item) => (
                        ...
                        >
                          ...
                          <button
                            onClick={() => openModal('confirm_payment', { orderId: item.id })}
                            className="w-full py-1.5 bg-indigo-600 text-white rounded-md text-xs font-bold hover:bg-indigo-700 transition-colors"
                          >
                            审核收款
                          </button>
                        </div>
                    ...
                    {pendingInvoices.length === 0 ? (
                      ...
                      pendingInvoices.map((item) => (
                        ...
                        >
                          ...
                          <button
                            onClick={() => {
                              openModal('issue_invoice', { orderId: item.id })
                            }}
                    ...
                    {pendingRefunds.length === 0 ? (
                      ...
                      pendingRefunds.map((refund) => (
                        ...
                        >
                          ...
                          <button
                            onClick={() =>
                              openModal('seller_refund_audit', { refundId: refund.id })
                            }
                            className="w-full py-1.5 bg-red-600 text-white rounded-md text-xs font-bold hover:bg-red-700 transition-colors"
                          >
                            审核处理
                          </button>
                        </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <div className="flex justify-between items-center mb-8 pl-2">
              ...
              <div className="flex items-center gap-4 text-xs text-gray-500">
                <span className="flex items-center gap-1.5">
                  <div className="w-2 h-2 rounded-full bg-green-500"></div> 总收款
                </span>
                <span className="flex items-center gap-1.5">
                  <div className="w-2 h-2 rounded-full bg-gray-200"></div> 平均值
                </span>
              </div>
            </div>
            <div className="h-64 flex items-end justify-between gap-2 md:gap-4 relative px-4">
              <div className="absolute inset-0 flex flex-col justify-between pointer-events-none">
                {[1, 2, 3, 4].map((l) => (
                  <div key={l} className="border-t border-dashed border-gray-100 w-full h-0"></div>
                ))}
              </div>
              {SELLER_REVENUE_CHART_DATA.map((d, i) => (
                <div
                  key={i}
                  className="flex-1 flex flex-col justify-end h-full gap-2 group cursor-pointer relative z-10"
                >
                  <div
            ...
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-gray-50 border-b border-gray-200 text-gray-500">
                <tr>
                  ...
                  <th className="px-6 py-4 font-medium text-right sticky right-0 bg-gray-50 z-10">
                    操作
                  </th>
                </tr>
              </thead>
                  ...
                  <tr key={item.id} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-gray-900 font-medium">
                        {item.asset}{' '}
                        <span className="text-gray-500 font-normal text-xs">({item.version})</span>
                      </div>
                    ...
                    <td className="px-6 py-4">
                      <div className="space-y-1 text-xs text-gray-600">
                        <div className="flex items-center justify-between gap-4">
                          <span>
                            Token:{' '}
                            <span className="font-mono text-gray-900">{item.usage.tokens}</span>
                          </span>
                          <span className="font-mono text-gray-900">
                            ¥ {item.feeBreakdown.tokens.toFixed(2)}
                          </span>
                        </div>
                        <div className="flex items-center justify-between gap-4">
                          <span>
                            存储:{' '}
                            <span className="font-mono text-gray-900">{item.usage.storage}</span>
                          </span>
                          <span className="font-mono text-gray-900">
                            ¥ {item.feeBreakdown.storage.toFixed(2)}
                          </span>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 font-mono font-bold text-indigo-600 text-base">
                    ...
                    <td className="px-6 py-4 text-right sticky right-0 bg-white group-hover:bg-gray-50/50 transition-colors z-10">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() => {
                            openModal('monitoring_detail', { itemId: item.id })
                          }}
                        ...
                        {item.plan === '按量计费' ? (
                          item.status === 'unbilled' ? (
                            <button
                              onClick={() => openModal('generate_invoice', { itemId: item.id })}
                              className="text-sm font-medium flex items-center gap-1.5 justify-center px-3 py-1.5 rounded-lg transition-colors text-indigo-600 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 shadow-sm"
                            >
                              <FileText size={14} /> 生成请款单
                            </button>
                          ) : item.status === 'under_review' ? (
                            <button
                              onClick={() => openModal('confirm_payment', { itemId: item.id })}
                              className="text-sm font-medium flex items-center gap-1.5 justify-center px-3 py-1.5 rounded-lg transition-colors text-blue-600 bg-blue-50 hover:bg-blue-100 border border-blue-200 shadow-sm"
                            >
                            ...
                            <button
                              disabled
                              className="text-sm font-medium flex items-center gap-1.5 justify-center px-3 py-1.5 rounded-lg transition-colors text-gray-400 cursor-not-allowed bg-gray-50"
                            >
                          ...
                          <span className="text-xs text-gray-400 flex items-center justify-end gap-1 px-3 py-1.5">
                            无需请款
                          </span>
                        )}
                      </div>
                ...
                {SELLER_TRANSACTIONS.map((trx, idx) => (
                  <tr key={idx} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-4">
                      <span
                        className={`text-xs px-2 py-1 rounded ${trx.type === 'income' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}
                      >
                    ...
                    <td className="px-4 py-4 text-center">
                      <span
                        className={`text-[10px] px-2 py-0.5 rounded border ${trx.status === 'settled' ? 'bg-green-50 text-green-600 border-green-100' : 'bg-gray-100 text-gray-500 border-gray-200'}`}
                      >
