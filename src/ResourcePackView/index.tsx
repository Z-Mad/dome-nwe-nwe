...
import { Package, Database, Cpu, HardDrive, CheckCircle, Search, Filter, AlertCircle, ShoppingCart, ChevronRight, X, Smartphone, CreditCard, Loader2, Info, Plus, Minus, FileText, HelpCircle, RefreshCcw, ShieldAlert, Trash2, ChevronDown, Building2, Download } from 'lucide-react';

interface ResourcePackViewProps {
    orders: any[]; // List of active orders to link to
    onPurchase: (items: CartItem[], targetOrderId: string, version: string, paymentMethod: string) => void;
}
...
const RULES_CONTENT = [
    {
        ...
        content: (
            <ul className="list-disc pl-5 space-y-2 text-xs text-gray-600 leading-relaxed">
                ...
                <li><strong>叠加规则：</strong>支持购买多个同类型或不同类型的资源包，额度将累加，有效期以各资源包独立时间为准。</li>
                <li><strong>适用范围：</strong>Token包通用于所有接入标准API的智能体；存储包仅限私有化部署或需独立数据库的实例。</li>
    ...
    },
    {
        ...
        content: (
            <ul className="list-disc pl-5 space-y-2 text-xs text-gray-600 leading-relaxed">
                <li><strong>抵扣顺序：</strong>系统优先抵扣即将过期的资源包额度（先到期先扣）。</li>
                <li><strong>混合支付：</strong>当资源包额度不足时，超出部分将自动按照“按量付费”标准从账户余额中扣除。</li>
                <li><strong>余额逻辑：</strong>平台没有充值功能，自动从已关联的企业对公账户或个人签约账户扣除。</li>
    ...
    },
    {
        ...
        content: (
            <ul className="list-disc pl-5 space-y-2 text-xs text-gray-600 leading-relaxed">
                <li><strong>五天无理由：</strong>购买后5天内且未使用任何额度，支持全额退款。</li>
                <li><strong>非全额退款：</strong>已使用部分额度或超过5天，不支持退款。特殊情况请提交工单申请，将收取15%手续费。</li>
    ...
    },
    {
        ...
        content: (
            <div className="space-y-2 text-xs text-gray-600 leading-relaxed">
                <p>1. 请在购买前确认您的实例运行状态正常，挂载至“已过期”或“已冻结”实例可能导致服务无法立刻恢复。</p>
                <p>2. 系统已自动开启<strong>“余额预警”</strong>功能。当资源包剩余额度低于20%时，系统将发送短信提醒，您可以在个人中心的订单管理中查看预警详情。</p>
                <p className="text-orange-600 font-bold">3. 严禁利用平台资源进行挖矿或其他违规计算行为，一经发现将封禁账号。</p>
            </div>
        )
    }
];
                ...
                <div className="flex flex-col lg:flex-row gap-8 items-start">
                    ...
                    <div className="flex-1 space-y-6 w-full">
                        ...
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                            <h3 className="font-bold text-gray-900 mb-4 text-base border-l-4 border-blue-600 pl-3">1. 产品类型 (Type)</h3>
                            <div className="flex gap-4">
                                {(Object.keys(CONFIG_OPTIONS) as Array<'token' | 'storage'>).map((type) => (
                                    <button
                                        key={type}
                                        onClick={() => { setSelectedType(type); setSelectedQuotaIdx(0); }}
                                    ...
                                    >
                                        ...
                                        {selectedType === type && <CheckCircle size={20} className="ml-auto text-blue-600 fill-blue-100" />}
                                    </button>
                                ))}
                            </div>
                        ...
                        {/* 2. Quota Selection */}
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                            <h3 className="font-bold text-gray-900 mb-4 text-base border-l-4 border-blue-600 pl-3">2. 资源规格 (Specification)</h3>
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                {currentConfig.quotas.map((quota, idx) => (
                                    ...
                                    >
                                        ...
                                        <div className={`text-sm font-bold mb-1 ${selectedQuotaIdx === idx ? 'text-blue-700' : 'text-gray-800'}`}>
                                            {quota.label}
                                        </div>
                        ...
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                            <h3 className="font-bold text-gray-900 mb-4 text-base border-l-4 border-blue-600 pl-3">3. 有效期与数量 (Duration & Quantity)</h3>
                        ...
                        <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-100 sticky top-6">
                            ...
                            <div className="mb-6 p-4 bg-gray-50 rounded-xl border border-gray-200">
                                ...
                                <div className="flex justify-between items-center mb-1">
                                    <span className="font-bold text-gray-800 text-sm">{currentQuota.label} / {currentDuration.label}</span>
                                    <span className="font-mono text-orange-600 font-bold">¥{currentSelectionSubtotal}</span>
                                </div>
                            ...
                            <div className="space-y-3 mb-6 max-h-[300px] overflow-y-auto custom-scrollbar pr-1">
                                {cartItems.length === 0 ? (
                                    ...
                                    cartItems.map(item => (
                                        <div key={item.id} className="p-3 border border-gray-200 rounded-xl relative group hover:border-blue-300 transition-colors bg-white">
                                            ...
                                            <div className="flex justify-between items-center text-xs text-gray-500">
                                                ...
                                                <span className="font-mono font-bold text-gray-700">¥{item.subtotal.toLocaleString()}</span>
                                            </div>
                            ...
                            <div className="border-t border-gray-100 pt-4">
                                <div className="flex justify-between items-end mb-4">
                                    ...
                                    <div className="text-right">
                                        <div className="text-3xl font-bold text-gray-900 font-mono text-orange-600 leading-none">
                                            <span className="text-sm mr-1">¥</span>{cartTotal.toLocaleString()}
                                        </div>
                                    </div>
                                </div>
                                ...
                                >
                                    去结算 <ChevronRight size={16} className="group-hover:translate-x-1 transition-transform"/>
                                </button>
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in">
                    <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                            ...
                            <button onClick={() => setIsPurchaseModalOpen(false)}><X size={20} className="text-gray-400 hover:text-gray-600"/></button>
                        </div>
                        
                        <div className="p-6 space-y-6 overflow-y-auto">
                            {/* Items Summary */}
                            <div className="bg-blue-50/50 rounded-xl border border-blue-100 p-4 max-h-40 overflow-y-auto custom-scrollbar">
                                <div className="text-xs font-bold text-blue-800 mb-2 uppercase">包含商品 ({cartItems.length})</div>
                                    ...
                                    {activeOrders.length > 0 ? activeOrders.map(order => (
                                        ...
                                        >
                                            <div>
                                                ...
                                                <div className="text-xs text-gray-500 font-mono">{order.id} · {order.provider}</div>
                                            </div>
                                            {targetOrderId === order.id && <CheckCircle size={18} className="text-blue-600"/>}
                                        </div>
                                    )) : (
                                        <div className="p-4 text-center text-sm text-gray-500 bg-gray-50 rounded-lg">暂无有效订阅实例</div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="flex gap-4">
                                    ...
                                    >
                                        <div className="w-5 h-5 bg-blue-500 rounded flex items-center justify-center text-white text-xs font-bold">支</div>
                                    ...
                                    >
                                        <div className="w-5 h-5 bg-green-500 rounded flex items-center justify-center text-white text-xs font-bold">微</div>
                                ...
                                {paymentMethod === 'offline' && (
                                    <div className="bg-orange-50 border border-orange-100 rounded-lg p-3 text-xs text-orange-800 mt-3">
                                        ...
                                        <p>请在提交订单后，下载<span className="text-blue-600 cursor-pointer hover:underline" onClick={() => setShowPaymentApplicationModal(true)}>《支付申请单》</span>并交由财务进行对公转账。款项到账后系统将自动开通服务。</p>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="p-4 bg-gray-50 border-t border-gray-100">
                            ...
                            <div className="flex items-start gap-2 mb-4 px-2">
                                ...
                                <label htmlFor="modal-agreement" className="text-xs text-gray-500 cursor-pointer select-none">
                                    ...
                                    <span 
                                        ...
                                        onClick={(e) => { e.preventDefault(); setShowAgreementDetail(true); }}
                                    >
                                        《资源包服务协议》
                                    </span>
                                    ，购买后立即生效，不支持退款。
                            ...
                            <div className="flex gap-3">
                                <button onClick={() => setIsPurchaseModalOpen(false)} className="flex-1 py-2.5 rounded-xl text-gray-600 font-bold hover:bg-gray-200 transition-colors">取消</button>
                                <button 
                                    onClick={paymentMethod === 'offline' ? () => { confirmPurchase(); setShowPaymentApplicationModal(true); } : confirmPurchase} 
                                ...
                                >
                                    {isProcessing ? (
                                        ...
                                        paymentMethod === 'offline' ? '提交订单并下载《支付申请单》' : `支付 ¥${cartTotal.toLocaleString()}`
                                    )}
                                </button>
                            </div>
                        </div>
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4 animate-in fade-in">
                    <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
                        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                            ...
                            <button onClick={() => setShowAgreementDetail(false)}><X size={20} className="text-gray-400 hover:text-gray-600"/></button>
                        </div>
                        <div className="p-6 overflow-y-auto text-sm text-gray-600 leading-relaxed space-y-4">
                            <p><strong>1. 服务内容</strong><br/>本资源包服务旨在为您提供额外的算力（Token）或存储空间。购买后，资源包将自动绑定至您指定的智能体实例，并优先于按量付费扣除。</p>
                            <p><strong>2. 有效期与过期</strong><br/>资源包具有明确的有效期。未在有效期内使用的额度将自动失效，不予退款或结转。请您根据实际需求合理规划购买量。</p>
                            <p><strong>3. 不可转让</strong><br/>资源包仅限当前账户下的实例使用，不支持跨账户转让或赠予。</p>
                            <p><strong>4. 免责声明</strong><br/>因不可抗力（如网络故障、服务器宕机）导致的服务中断，平台将依据SLA条款进行赔偿，但不承担超出资源包价值的连带责任。</p>
                        </div>
                        <div className="p-4 border-t border-gray-100 bg-gray-50 text-right">
                            <button 
                                onClick={() => { setShowAgreementDetail(false); setAgreementChecked(true); }} 
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[70] flex items-center justify-center p-4 animate-in fade-in">
                    <div className="bg-white w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                            ...
                            <button onClick={() => setShowPaymentApplicationModal(false)} className="text-gray-400 hover:text-gray-600 transition-colors">
                                <X size={20} />
                            </button>
                        </div>
                        ...
                        <div className="p-8 overflow-y-auto flex-1 bg-gray-50">
                            <div className="bg-white p-10 shadow-sm border border-gray-200 mx-auto max-w-xl" style={{ minHeight: '600px' }}>
                                ...
                                <div className="flex justify-between text-sm mb-6">
                                    <div><span className="text-gray-500">申请日期：</span> {new Date().toLocaleDateString()}</div>
                                    <div><span className="text-gray-500">订单编号：</span> ORD-{new Date().getFullYear()}{String(new Date().getMonth() + 1).padStart(2, '0')}{String(new Date().getDate()).padStart(2, '0')}-{Math.floor(Math.random() * 1000).toString().padStart(3, '0')}</div>
                                </div>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td className="border border-gray-300 p-3">
                                                <span className="font-bold text-lg">¥ {cartTotal.toLocaleString('zh-CN', {minimumFractionDigits: 2})}</span>
                                            </td>
                                        </tr>
                                        <tr>
