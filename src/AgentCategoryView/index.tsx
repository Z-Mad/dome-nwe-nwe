    ...
    return (
      <div className="flex-1 bg-gray-50 overflow-y-auto h-full">
        ...
        <div className="bg-black text-white h-64 relative overflow-hidden flex items-center">
          ...
          <img
            src="https://picsum.photos/seed/service_hero_steel/1600/600"
            className="absolute inset-0 w-full h-full object-cover opacity-50"
            alt="Hero"
          />
          <div className="container mx-auto px-8 relative z-20">
            <span className="text-blue-300 text-xs font-bold tracking-wider mb-2 block">
              维观推荐
            </span>
            <h1 className="text-4xl font-bold mb-4">钢铁供应链增值服务</h1>
            <p className="text-gray-300 max-w-lg text-sm">
              从AI能力到实体交易。连接金融、维保、物流的钢铁产业生态。
            </p>
          </div>
        </div>
              ...
              <div className="space-y-1">
                {serviceCategories.map((cat) => (
                  ...
                  >
                    <span>{cat.label}</span>
                    <span
                      className={`text-xs ${serviceSubCategory === cat.id ? 'text-yellow-600' : 'text-gray-400'}`}
                    >
                      {cat.count}
                    </span>
                  </button>
          ...
          <div className="flex-1">
            ...
            <div className="space-y-4">
              {filteredServices.map((service) => (
                <div
                  key={service.id}
                  onClick={() => onNavigateToDetail(service.id.toString())}
                    ...
                    <div className="md:text-right flex flex-col items-start md:items-end justify-between self-stretch min-w-[140px]">
                      ...
                      <div className="mt-4 flex flex-col items-end gap-2">
                        <div className="text-xs text-gray-500 flex items-center gap-1">
                          {service.provider}
                          <img
                            src={`https://picsum.photos/seed/${service.provider}/20/20`}
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div
              key={i}
              onClick={() => onNavigateToDetail(i.toString())}
              className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 hover:shadow-xl hover:-translate-y-1 transition-all cursor-pointer group"
            >
              <div className="flex justify-between items-start mb-4">
                <div
                  className={`w-10 h-10 rounded-xl flex items-center justify-center ${category === 'method' ? 'bg-blue-50 text-blue-600' : 'bg-purple-50 text-purple-600'}`}
                >
              ...
              <div className="flex justify-between items-center text-xs text-gray-400 pt-4 border-t border-gray-50">
                <span className="flex items-center gap-1">
                  <Download size={12} /> {100 * i + 23}
                </span>
                <span className="flex items-center gap-1 text-yellow-500">
                  <Star size={12} fill="#eab308" /> 4.8
                </span>
              </div>
            </div>
          ))}
