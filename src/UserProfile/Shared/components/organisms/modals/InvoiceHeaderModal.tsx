        ...
        <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
          <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
            <h3 className="font-bold text-gray-900 flex items-center gap-2">
              ...
              {editingInvoiceHeader.id ? '编辑发票抬头' : '新增发票抬头'}
            </h3>
            <button
              onClick={() => setShowInvoiceHeaderForm(false)}
          ...
          <div className="p-6 overflow-y-auto flex-1 space-y-4">
            <div>
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="radio"
                    checked={editingInvoiceHeader.type === 'enterprise'}
                    onChange={() =>
                      setEditingInvoiceHeader({ ...editingInvoiceHeader, type: 'enterprise' })
                    }
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="radio"
                    checked={editingInvoiceHeader.type === 'special'}
                    onChange={() =>
                      setEditingInvoiceHeader({ ...editingInvoiceHeader, type: 'special' })
                    }
                    className="text-blue-600 focus:ring-blue-500"
                  />
            ...
            <div>
              ...
              <input
                type="text"
                value={editingInvoiceHeader.title}
                onChange={(e) =>
                  setEditingInvoiceHeader({ ...editingInvoiceHeader, title: e.target.value })
                }
            ...
            <div>
              ...
              <input
                type="text"
                value={editingInvoiceHeader.taxId}
                onChange={(e) =>
                  setEditingInvoiceHeader({ ...editingInvoiceHeader, taxId: e.target.value })
                }
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
            ...
            {editingInvoiceHeader.type === 'special' && (
              <>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    注册地址及电话 <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="text"
                    value={editingInvoiceHeader.address || ''}
                    onChange={(e) =>
                      setEditingInvoiceHeader({ ...editingInvoiceHeader, address: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    开户行及账号 <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="text"
                    value={editingInvoiceHeader.bank || ''}
                    onChange={(e) =>
                      setEditingInvoiceHeader({ ...editingInvoiceHeader, bank: e.target.value })
                    }
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all"
            ...
            <div className="pt-2">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  checked={editingInvoiceHeader.isDefault}
                  onChange={(e) =>
            ...
            <button
              onClick={() => {
                ...
                if (editingInvoiceHeader.id) {
                  setInvoiceHeaders(
                    newHeaders.map((h) =>
                      h.id === editingInvoiceHeader.id ? editingInvoiceHeader : h,
                    ),
                  )
                  showToast('修改成功')
                } else {
                  setInvoiceHeaders([
                    ...newHeaders,
                    { ...editingInvoiceHeader, id: `h${Date.now()}` },
                  ])
                  showToast('新增成功')
                }
      ...
      <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-4">
          {invoiceHeaders.map((header) => (
            <div
              key={header.id}
              className="border border-gray-200 rounded-xl p-4 relative hover:border-blue-300 transition-colors"
            >
              {header.isDefault && (
                <span className="absolute top-4 right-4 bg-blue-100 text-blue-700 text-xs px-2 py-1 rounded font-bold">
                  默认
                </span>
              )}
              ...
              <div className="grid grid-cols-2 gap-2 text-sm text-gray-600">
                ...
                <div>
                  ...
                  {header.type === 'special' ? '增值税专用发票' : '增值税普通发票'}
                </div>
                <div className="col-span-2">
                  <span className="text-gray-400">开户行及账号：</span>
                  {header.bank || '-'}
                </div>
              </div>
              <div className="mt-4 flex gap-3">
                <button
                  onClick={() => {
                    setEditingInvoiceHeader(header)
...
export default InvoiceHeaderModal
