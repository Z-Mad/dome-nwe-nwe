// pages/profile/buyer/Invoices.tsx
...
const BuyerInvoices: React.FC = () => {
  const { openModal, globalOrders, bills, invoices, invoiceSubTab, setInvoiceSubTab } = useOutletContext<any>();

  // 可开票数据（从已支付且未开票的订单/账单中提取）
  const invoiceableItems = [
    ...bills
      .filter((b: any) => b.status === 'paid' && b.invoiceStatus === 'unissued')
      .map((b: any) => ({ id: b.id, period: b.period, amount: b.amount, type: 'bill' })),
    ...globalOrders
      .filter((o: any) => o.paymentStatus === 'Paid' && o.invoiceStatus === 'Unissued' && o.amount > 0)
      .map((o: any) => ({ id: o.id, productName: o.productName, amount: o.amount, type: 'order' })),
  ];
    ...
    <div className="space-y-6 animate-in fade-in">
      <div className="flex justify-between items-center mb-2">
        ...
        <div className="flex gap-3">
          <button
            onClick={() => openModal('invoice_header', null)}
              ...
              {invoiceableItems.map((item) => (
                <tr key={item.id} className="hover:bg-gray-50 transition-colors group">
                  ...
                  <td className="px-6 py-4 text-right">
                    <button
                      onClick={() => openModal('request_invoice', item)}
