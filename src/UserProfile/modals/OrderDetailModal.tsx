...
export const OrderDetailModal: React.FC<OrderDetailModalProps> = ({
  ...
  const isTrialSuspended = order.status === 'TRIAL_SUSPENDED';
  const isExpired = order.status === 'Expired';
  const isActive = order.status === 'Active';
  const isPaymentFailed = order.status === 'PaymentFailed';

  // Mock installation fee logic (sync with publish wizard config)
  const orderType = order.orderType || 'NewPurchase';
  const isTrialOrder = orderType === 'Trial' || order.status === 'Trial';
  const isUsageOrder = order.snapshot?.period === 'Usage';
  const isResourcePackOrder = orderType === 'ResourcePack';

  let installationFee = 0;
  if (isTrialOrder || isUsageOrder) {
    installationFee = order.amount;
  } else if (!isResourcePackOrder) {
    installationFee = 2000; // Mock amount for standard subscriptions
  }

  const baseAmount = Math.max(0, order.amount - installationFee);

  const getTimelineSteps = () => {
    const steps = [
      { label: '订单创建', time: order.createTime, active: true, color: 'bg-blue-500' },
    ];

    if (order.paymentStatus === 'Cancelled') {
      steps.push({ label: '订单取消', time: '-', active: true, color: 'bg-gray-500' });
    } else if (order.paymentStatus === 'PendingPayment') {
      steps.push({ label: '等待支付', time: '-', active: true, color: 'bg-orange-500' });
    } else if (order.paymentStatus === 'UnderReview') {
      steps.push({ label: '提交回执', time: order.payTime || '-', active: true, color: 'bg-blue-500' });
    ...
    } else if (order.paymentStatus === 'PaymentFailed') {
      steps.push({ label: '支付失败', time: order.payTime || '-', active: true, color: 'bg-red-500' });
    } else {
      // Paid
      steps.push({ label: '支付成功', time: order.payTime || '-', active: true, color: 'bg-blue-500' });
      steps.push({
        label: '订单完成',
        time: order.status === 'Active' || order.status === 'Running' ? (order.payTime || order.createTime) : '-',
        active: order.status === 'Active' || order.status === 'Running',
        color: order.status === 'Active' || order.status === 'Running' ? 'bg-green-500' : 'bg-gray-200',
      });
    }
      ...
      <div className="bg-white w-full max-w-2xl h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
          ...
          <div className="max-w-3xl mx-auto space-y-6">
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              <div>
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <StatusBadge status={order.status} expireDate={order.expireDate} />
                </div>
                <div className="text-sm text-gray-500 font-mono">订单号: {order.id}</div>
              </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              ...
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  <div className="font-bold text-lg text-gray-900">{order.productName}</div>
                  <div className="text-sm text-gray-500">
                    供应商: {order.provider} | 版本: {order.version}
                  </div>
                </div>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 bg-gray-50 rounded-xl p-4 border border-gray-100">
                ...
                <div>
                  ...
                  <div className="font-bold text-gray-900 text-sm">
                    {isSubscription
                      ? order.snapshot?.period === 'Yearly'
                        ? '包年'
                        : order.snapshot?.period === 'Monthly'
                          ? '包月'
                          : '按量付费'
                      : '一次性付费'}
                  </div>
                ...
                <div>
                  ...
                  <div className="font-bold text-gray-900 text-sm font-mono">{order.createTime}</div>
                </div>
                <div>
                  <div className="text-xs text-gray-400 mb-1">服务周期</div>
                  <div className="font-bold text-gray-900 text-sm font-mono">
                    {order.expireDate
                      ? `${order.createTime.split(' ')[0]} 至 ${order.expireDate.split(' ')[0]}`
                      : '-'}
                  </div>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                ...
                <div className="space-y-3">
                  ...
                  {order.snapshot?.quota && (
                    <div className="bg-gray-50 p-3 rounded-lg border border-gray-100 mt-2">
                      <div className="text-xs text-gray-500 mb-2">包含明细：</div>
                      <div className="space-y-1.5">
                        {order.snapshot.quota.tokens && (
                          <div className="flex justify-between text-xs">
                            <span className="text-gray-600">Token 额度</span>
                            <span className="font-medium text-gray-900">
                              {(order.snapshot.quota.tokens / 10000).toFixed(0)}万 Tokens
                            </span>
                          </div>
                        )}
                        {order.snapshot.quota.storage && (
                          <div className="flex justify-between text-xs">
                            <span className="text-gray-600">存储空间</span>
                            <span className="font-medium text-gray-900">
                              {order.snapshot.quota.storage} GB
                            </span>
                          </div>
                        )}
                        {order.snapshot.quota.users && (
                          <div className="flex justify-between text-xs">
                            <span className="text-gray-600">用户数</span>
                            <span className="font-medium text-gray-900">
                              {order.snapshot.quota.users} 个
                            </span>
                          </div>
                        )}
                        {order.snapshot.plan && (
                          <div className="flex justify-between text-xs">
                            ...
                            <span className="font-medium text-gray-900">{order.snapshot.plan}</span>
                          </div>
                        )}
                      </div>
                    </div>
                ...
                <div className="space-y-3">
                    ...
                    <>
                      <div className="flex justify-between items-center text-sm">
                        ...
                        <span
                          ...
                          onClick={() => onOpenModal('preview_image', order)}
                        >
                  ...
                  {order.rejectReason && (
                    <div className="mt-2 text-xs text-red-600 bg-red-50 p-2 rounded border border-red-100">
                      <span className="font-bold">驳回原因:</span> {order.rejectReason}
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center justify-between relative px-4 sm:px-12">
                ...
                {timelineSteps.map((step, index) => (
                  <div key={index} className="relative z-10 flex flex-col items-center bg-white px-2">
                    <div
                      className={`w-4 h-4 rounded-full border-2 border-white shadow-sm mb-2 ${
                        step.active ? step.color : 'bg-gray-200'
                      }`}
                    ></div>
                    <div className={`text-xs font-bold ${step.active ? 'text-gray-900' : 'text-gray-400'}`}>
                      {step.label}
                    </div>
        ...
        <div className="p-4 border-t border-gray-100 bg-white flex justify-between items-center flex-wrap gap-4">
          ...
          <div className="flex gap-3 flex-wrap justify-end">
            {order.paymentStatus === 'Paid' &&
              (order.orderType === 'Trial' || order.orderType === 'New') && (
                <button
                  onClick={() => {
                    onClose();
                    onNavigate('detail', {
            ...
            {isActive && isSubscription && (
              <>
                ...
                <button
                  onClick={() => {
                    onClose();
                    onOpenModal('refund_request', order);
                  }}
