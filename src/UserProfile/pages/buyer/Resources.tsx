...
import { useBuyerResources } from '../../hooks/useBuyerResources';

const BuyerResources: React.FC = () => {
  const { globalResources, onNavigate, onUpdateResource, showToast } = useOutletContext<any>();
  const {
    localResources,
    resourceSubTab,
    setResourceSubTab,
    ...
    updateResource,
  } = useBuyerResources(globalResources, onUpdateResource);
                        ...
                        <button
                          onClick={() => {
                            if (onUpdateResource && editingInstanceName.trim()) {
                              updateResource(resource.id, { instanceName: editingInstanceName.trim() });
                              showToast('实例名称已更新');
                            }
