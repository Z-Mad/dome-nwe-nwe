...
const RelatedPanel: React.FC = () => {
  const getIcon = (iconName: string) => {
    switch (iconName) {
      case 'factory':
        return <Factory size={18} className="text-blue-500" />
      case 'list':
      ...
      case 'shield':
        return <ShieldCheck size={18} className="text-purple-500" />
      default:
    ...
    <div className="w-72 bg-gray-50/50 border-l border-gray-200 h-full flex flex-col p-4">
      ...
      <div className="space-y-3">
        {RELATED_RECORDS.map((record) => (
          <div
            key={record.id}
            className="bg-white p-3 rounded-xl shadow-sm border border-gray-100 hover:shadow-md transition-all cursor-pointer flex items-center gap-3"
          >
            <div
              className={`w-10 h-10 rounded-lg ${getBg(record.icon)} flex items-center justify-center flex-shrink-0`}
            >
              {getIcon(record.icon)}
            </div>
