...
import { Headphones, MessageSquare, HelpCircle, ChevronRight } from 'lucide-react';
import { SUPPORT_TICKETS, FAQ_ITEMS } from '../../constants/profile';

interface BuyerSupportProps {
  onNavigate: (view: string, params?: any) => void;
  onOpenModal: (type: string, item?: any) => void;
}

export const BuyerSupport: React.FC<BuyerSupportProps> = ({ onNavigate, onOpenModal }) => {
        ...
        <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm flex flex-col">
          <div className="flex justify-between items-center mb-6">
            <h3 className="font-bold text-gray-900 flex items-center gap-2">
              <MessageSquare size={20} className="text-orange-500" /> 工单中心
            </h3>
            <button
              onClick={() => onOpenModal('create_ticket')}
                ...
                <div className="flex justify-between items-start mb-1">
                  ...
                  <span
                    className={`text-[10px] px-2 py-0.5 rounded ${
                      ticket.status === 'processing' ? 'bg-blue-50 text-blue-600' : 'bg-gray-100 text-gray-500'
                    }`}
                  >
