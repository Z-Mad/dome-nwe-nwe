...
import { SELLER_STATS, SELLER_REVENUE_CHART_DATA } from '../../constants/profile';

const SellerDashboard: React.FC = () => {
  const { setSellerTab, onNavigate } = useOutletContext<any>();
    ...
    <div className="space-y-6 animate-in fade-in">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 group hover:border-purple-200 transition-colors">
          <div className="flex justify-between items-start mb-2">
            <div className="text-gray-500 text-xs font-bold uppercase tracking-wider">API 调用量</div>
        ...
        <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 group hover:border-green-200 transition-colors">
          <div className="flex justify-between items-start mb-2">
            <div className="text-gray-500 text-xs font-bold uppercase tracking-wider">服务健康度</div>
            <div className="bg-green-50 text-green-600 text-[10px] px-1.5 py-0.5 rounded font-bold">运行正常</div>
          </div>
        ...
        <div className="flex-1 bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="flex items-center gap-4 text-xs">
              <div className="flex items-center gap-1.5">
                <div className="w-2.5 h-2.5 rounded-sm bg-green-500"></div>月度收款
              </div>
            </div>
          </div>
          ...
          <div className="h-64 flex items-end justify-between gap-4 relative">
            ...
            {SELLER_REVENUE_CHART_DATA.map((d, i) => (
              <div key={i} className="flex-1 flex flex-col justify-end group h-full relative z-10 cursor-pointer">
                <div
        ...
        <div className="w-full lg:w-80 space-y-6">
          <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                onClick={() => onNavigate('publish_wizard', { mode: 'create' })}
              ...
              {[
                ...
                <div key={i} className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${
                        i === 0 ? 'bg-yellow-100 text-yellow-700' : i === 1 ? 'bg-gray-100 text-gray-600' : 'bg-orange-50 text-orange-700'
                      }`}
                    >
                      {i + 1}
                    </div>
                    <span className="text-xs font-bold text-gray-700 truncate w-24">{item.name}</span>
                  </div>
                  <div className="text-right">
                    ...
                    >
                      {item.trend === 'up' ? <ArrowUpRight size={8} /> : <ArrowUpRight size={8} className="rotate-180" />}
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between p-3 hover:bg-gray-50 rounded-xl transition-colors cursor-pointer border border-transparent hover:border-gray-100">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="text-sm font-bold text-gray-900">新订单: 冷轧板形控制专家 (企业版)</div>
          ...
          >
            ...
            <div className="text-right">
              ...
              <button className="text-[10px] text-blue-600 font-bold bg-blue-50 px-2 py-0.5 rounded mt-1">去处理</button>
            </div>
          </div>
