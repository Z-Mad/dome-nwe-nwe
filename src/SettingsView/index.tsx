...
import { 
  User, Bell, Lock, Globe, Moon, Shield, Key, 
  CheckCircle, Trash2, Plus, Copy, AlertTriangle, 
...
const SettingsView: React.FC<SettingsViewProps> = ({ currentAccount, accounts, onSwitchAccount }) => {
  ...
  const [apiKeys, setApiKeys] = useState<ApiKey[]>([
    { id: 'k1', name: 'ERP Integration', prefix: 'sk-prod-7f8a...', environment: 'production', ipWhitelist: '10.20.1.5', expiration: '2025-12-31', lastUsed: '2 mins ago', status: 'active' },
    { id: 'k2', name: 'Dev Sandbox', prefix: 'sk-test-9b2c...', environment: 'sandbox', ipWhitelist: 'Any', expiration: '2025-06-30', lastUsed: '1 day ago', status: 'active' }
  ]);
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
            ...
            <div className="flex items-start gap-6 mb-8">
                <div className="relative group">
                    <div className="w-20 h-20 rounded-full bg-gray-100 overflow-hidden ring-4 ring-gray-50 border border-gray-200">
                        <img src={currentAccount.avatar} className="w-full h-full rounded-full object-cover" />
                    </div>
                    {!isViewer && (
                ...
                <div className="flex-1 space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1.5">昵称 / Display Name</label>
                            <input 
                                type="text" 
                         ...
                         <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1.5">绑定邮箱 / Email</label>
                        ...
                        <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1.5">联系电话 / Phone</label>
                            <input 
                                type="text" 
                                value={profile.phone} 
       ...
       <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            ...
            <div className="flex flex-col md:flex-row gap-6">
                <div className="flex-1 space-y-4">
                    ...
                    <div>
                        <label className="block text-xs font-bold text-gray-500 mb-1.5">统一社会信用代码</label>
                        <input 
                            type="text" 
            ...
            {profile.isVerified && (
                <div className="mt-4 bg-green-50 border border-green-100 rounded-lg p-3 flex gap-2 items-start">
                    ...
                    <div className="text-xs text-green-800">
                        ...
                        <p className="mt-0.5 opacity-80">企业认证信息已通过平台审核，享有增值税专票开具及大额支付权限。</p>
                    </div>
                </div>
            )}
       </div>
       ...
       {!isViewer && (
           <div className="flex justify-end">
               <button onClick={handleSaveProfile} className="bg-blue-600 text-white px-6 py-2.5 rounded-xl font-bold shadow-lg shadow-blue-200 hover:bg-blue-700 transition-colors flex items-center gap-2">
                   <Check size={18}/> 保存修改
               </button>
           </div>
       )}
            ...
            <div className="space-y-6 divide-y divide-gray-50">
                ...
                <div className="flex justify-between items-center py-2">
                    <div>
                        ...
                        <div className="text-xs text-gray-500">
                            上次修改时间：<span className="font-mono text-gray-700">{securityState.lastPasswordChange}</span>
                        </div>
                    </div>
                ...
                <div className="flex justify-between items-center py-4">
                    <div className="flex items-start gap-3">
                        <div className={`mt-1 p-1.5 rounded-lg ${securityState.mfaEnabled ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-400'}`}>
                            <Smartphone size={18}/>
                        </div>
                        <div>
            ...
            <div className="space-y-1">
                ...
                <div className="flex items-center justify-between py-4 border-b border-gray-50 opacity-70">
                    <div>
                        ...
                        <div className="text-xs text-gray-500 mt-0.5">异地登录、密码修改、API Key 操作等安全通知</div>
                    </div>
                ...
                <div className="flex items-center justify-between py-4 border-b border-gray-50 opacity-70">
                    <div>
                        ...
                        <div className="text-xs text-gray-500 mt-0.5">订单支付、退款进度、余额预警、发票开具通知</div>
                    </div>
                    <div className="flex items-center gap-2 text-xs font-bold text-gray-400">
                        <CheckCircle size={14} className="text-gray-400"/> 已锁定开启
                    </div>
                ...
                {[
                    { key: 'marketing', label: '营销推广 (Marketing)', desc: '接收最新的产品活动、优惠券发放及行业动态' },
                    { key: 'productUpdate', label: '产品更新 (Updates)', desc: '关注的智能体版本更新、新功能上线提醒' },
                ].map((item) => (
                    <div key={item.key} className="flex items-center justify-between py-4 border-b border-gray-50 last:border-0">
                        ...
                        >
                            <div className={`w-4 h-4 bg-white rounded-full absolute top-1 shadow-sm transition-transform duration-200 ${
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-right-4 duration-300">
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            ...
            <div className="space-y-3">
                {apiKeys.map(key => (
                    <div key={key.id} className="border border-gray-200 rounded-xl p-4 flex items-center justify-between hover:bg-gray-50 transition-colors">
                        <div>
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className={`text-[10px] px-1.5 py-0.5 rounded border uppercase font-bold ${
                                    key.environment === 'production' ? 'bg-orange-50 text-orange-600 border-orange-100' : 'bg-blue-50 text-blue-600 border-blue-100'
                                }`}>
                                    {key.environment === 'production' ? 'Prod' : 'Sandbox'}
                                </span>
                                {key.status === 'active' ? (
                                    <span className="text-[10px] bg-green-50 text-green-600 px-1.5 py-0.5 rounded border border-green-100">Active</span>
                                ) : (
                                    <span className="text-[10px] bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded border border-gray-200">Expired</span>
                                )}
                            </div>
                            <div className="flex items-center gap-3 text-xs text-gray-500">
                                <div className="font-mono bg-gray-100 px-1.5 rounded">{key.prefix}****************</div>
      ...
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden flex flex-col">
              <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                  ...
                  <button onClick={() => setShowKeyModal(false)}><X size={20} className="text-gray-400 hover:text-gray-600"/></button>
              </div>
              ...
              <div className="p-6">
                  {!generatedKey ? (
                      <div className="space-y-4">
                          <div>
                              <label className="block text-xs font-bold text-gray-500 mb-1.5">密钥名称 (Name)</label>
                              <input 
                                  type="text" 
                          ...
                          <div className="grid grid-cols-2 gap-4">
                              <div>
                                  <label className="block text-xs font-bold text-gray-500 mb-1.5">环境 (Environment)</label>
                                  <select 
                              ...
                              <div>
                                  <label className="block text-xs font-bold text-gray-500 mb-1.5">有效期 (Expiration)</label>
                                  <select 
                          ...
                          <div>
                              <label className="block text-xs font-bold text-gray-500 mb-1.5">IP 白名单 (Optional)</label>
                              <input 
                                  type="text" 
                                  ...
                                  className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:border-blue-500 outline-none"
                              />
                              <p className="text-[10px] text-gray-400 mt-1">为了安全起见，建议仅允许可信服务器的 IP 地址访问生产环境 Key。</p>
                          </div>
                      ...
                      <div className="space-y-4 animate-in fade-in">
                          <div className="bg-green-50 border border-green-100 rounded-xl p-4 text-center">
                              ...
                              <p className="text-xs text-green-700 mt-1">请立即复制并保存，离开此页面后将无法再次查看。</p>
                          </div>
                          ...
                          <div>
                              ...
                              <div className="flex gap-2">
                                  ...
                                  <button 
                                      onClick={() => { navigator.clipboard.writeText(generatedKey); triggerToast('已复制到剪贴板'); }}
            ...
            <div className="w-full md:w-64 flex-shrink-0 space-y-2 sticky top-6 self-start">
                <button 
                    ...
                    className={`w-full text-left px-4 py-3 rounded-xl font-medium transition-all flex items-center gap-3 ${
                        activeTab === 'profile' ? 'bg-white shadow-sm text-blue-600 border border-blue-100' : 'text-gray-600 hover:bg-white/60 hover:text-gray-900'
                    }`}
                >
                    <User size={18}/> 个人资料
                </button>
                <button 
                    ...
                    className={`w-full text-left px-4 py-3 rounded-xl font-medium transition-all flex items-center gap-3 ${
                        activeTab === 'security' ? 'bg-white shadow-sm text-blue-600 border border-blue-100' : 'text-gray-600 hover:bg-white/60 hover:text-gray-900'
                    }`}
                >
                    <Lock size={18}/> 账号安全
                </button>
                <button 
                    ...
                    className={`w-full text-left px-4 py-3 rounded-xl font-medium transition-all flex items-center gap-3 ${
                        activeTab === 'notifications' ? 'bg-white shadow-sm text-blue-600 border border-blue-100' : 'text-gray-600 hover:bg-white/60 hover:text-gray-900'
                    }`}
                >
                    <Bell size={18}/> 通知偏好
                </button>
                ...
                {!isViewer && (
                    <button 
                        ...
                        className={`w-full text-left px-4 py-3 rounded-xl font-medium transition-all flex items-center gap-3 ${
                            activeTab === 'api_keys' ? 'bg-white shadow-sm text-blue-600 border border-blue-100' : 'text-gray-600 hover:bg-white/60 hover:text-gray-900'
                        }`}
                    >
                        <Key size={18}/> API 密钥
                    </button>
