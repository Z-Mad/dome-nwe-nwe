...
const SELLER_REVENUE_CHART_DATA = [
...
const INITIAL_BILLS = [
  {
    ...
    details: [
      ...
      },
      {
        ...
        amount: 4000.0,
        usageItems: [
          { name: "月度分摊费", usage: "1个月", amount: 4000.0 }
        ]
      },
    ],
  },
...
// Mock Full Asset Data for Seller Console
const RICH_ASSETS_MOCK = [
  {
    id: "1",
    title: "冷轧板形控制专家",
...
const UserProfile: React.FC<UserProfileProps> = ({
  ...
  const renderOrderDetailModal = () => {
    ...
    const formatQuota = (quota: any) => {
      ...
      if (typeof quota === "object") {
        const parts = [];
        if (quota.tokens)
          parts.push(`${(quota.tokens / 1000).toFixed(0)}k Tokens`);
    ...
    const getTimelineSteps = () => {
      ...
      if (selectedItem.paymentStatus === "Cancelled") {
        ...
        steps.push({ label: "提交回执", time: selectedItem.payTime || "-", active: true, color: "bg-blue-500" });
        ...
        steps.push({ label: "支付失败", time: selectedItem.payTime || "-", active: true, color: "bg-red-500" });
      } else {
        // Paid
        steps.push({ label: "支付成功", time: selectedItem.payTime || "-", active: true, color: "bg-blue-500" });
        steps.push({ 
          label: "订单完成", 
          time: selectedItem.status === 'Active' || selectedItem.status === 'Running' ? (selectedItem.payTime || selectedItem.createTime) : '-', 
          active: selectedItem.status === 'Active' || selectedItem.status === 'Running', 
          color: selectedItem.status === 'Active' || selectedItem.status === 'Running' ? "bg-green-500" : "bg-gray-200" 
        });
      }
        ...
        <div className="bg-white w-full max-w-2xl h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
          ...
          <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-white">
            <h3 className="font-bold text-lg text-gray-900 flex items-center gap-2">
              <FileText size={20} className="text-blue-600" /> 订单管理 (Order
              Management)
            </h3>
            <button
              onClick={closeModal}
                ...
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    <h2 className="text-2xl font-bold text-gray-900">
                      {selectedItem.paymentStatus === "Paid" ? "订单已支付" : selectedItem.paymentStatus === "PendingPayment" ? "等待支付" : selectedItem.paymentStatus === "UnderReview" ? "审核中" : selectedItem.paymentStatus === "Rejected" ? "已驳回" : selectedItem.paymentStatus === "PaymentFailed" ? "支付失败" : "已取消"}
                    </h2>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 bg-gray-50 rounded-xl p-4 border border-gray-100">
                  <div>
                    ...
                    <div className="font-bold text-gray-900 text-sm">
                      {selectedItem.orderType === "Trial" ? "试用订单" : selectedItem.orderType === "Renewal" ? "续费订单" : selectedItem.orderType === "ResourcePack" ? "扩展资源包订单" : "新购订单"}
                    </div>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <div className="font-bold text-gray-900 text-sm font-mono">
                      {selectedItem.expireDate ? `${selectedItem.createTime.split(' ')[0]} 至 ${selectedItem.expireDate.split(' ')[0]}` : "-"}
                    </div>
                  </div>
                </div>
              </div>
                        ...
                        <div className="space-y-1.5">
                          {selectedItem.snapshot.quota.tokens && (
                            <div className="flex justify-between text-xs">
                              ...
                              <span className="font-medium text-gray-900">{(selectedItem.snapshot.quota.tokens / 10000).toFixed(0)}万 Tokens</span>
                            </div>
                          )}
                          {selectedItem.snapshot.quota.storage && (
                            <div className="flex justify-between text-xs">
                              <span className="text-gray-600">存储空间</span>
                              <span className="font-medium text-gray-900">{selectedItem.snapshot.quota.storage} GB</span>
                            </div>
                          )}
                          {selectedItem.snapshot.quota.users && (
                            <div className="flex justify-between text-xs">
                              <span className="text-gray-600">用户数</span>
                              <span className="font-medium text-gray-900">{selectedItem.snapshot.quota.users} 个</span>
                            </div>
                          )}
                          {selectedItem.snapshot.plan && (
                            <div className="flex justify-between text-xs">
                              <span className="text-gray-600">套餐类型</span>
                              <span className="font-medium text-gray-900">{selectedItem.snapshot.plan}</span>
                            </div>
                          )}
                        </div>
                      </div>
                ...
                <div className="flex items-center justify-between relative px-4 sm:px-12">
                  ...
                  {timelineSteps.map((step, index) => (
                    <div key={index} className="relative z-10 flex flex-col items-center bg-white px-2">
                      <div className={`w-4 h-4 rounded-full border-2 border-white shadow-sm mb-2 ${step.active ? step.color : 'bg-gray-200'}`}></div>
                      <div className={`text-xs font-bold ${step.active ? 'text-gray-900' : 'text-gray-400'}`}>{step.label}</div>
          ...
          <div className="p-4 border-t border-gray-100 bg-white flex justify-between items-center flex-wrap gap-4">
            ...
            <div className="flex gap-3 flex-wrap justify-end">
              {selectedItem.paymentStatus === "Paid" && (selectedItem.orderType === "Trial" || selectedItem.orderType === "New") && (
                <button
                  onClick={() => {
                    closeModal();
                    onNavigate("detail", {
                        ...
                        <tr className="bg-gray-50/30">
                          <td colSpan={4} className="px-6 py-3 border-t border-gray-50">
                            <div className="pl-4 border-l-2 border-blue-200">
                              <div className="text-xs font-bold text-gray-500 mb-2 uppercase tracking-wider">资源消耗明细</div>
                                ...
                                {detail.usageItems.map((item: any, itemIdx: number) => (
                                  <div key={itemIdx} className="flex justify-between items-center text-sm">
                                    <div className="flex items-center gap-4">
                                      <span className="text-gray-700 font-medium w-32">{item.name}</span>
                                      <span className="text-gray-500 text-xs bg-white px-2 py-0.5 rounded border border-gray-200">{item.usage}</span>
                                    </div>
  ...
  const renderRefundRequestModal = () => {
    if (!selectedItem) return null;
    ...
    const penaltyRatio = 0.15; // 15% penalty
    const baseRefund = selectedItem.amount * (1 - usageRatio);
    const penalty = selectedItem.amount * penaltyRatio;
    ...
    return (
      <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] flex justify-end animate-in fade-in">
        <div className="bg-white w-full max-w-xl h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
          <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-white">
            <h3 className="font-bold text-lg text-gray-900 flex items-center gap-2">
              <RefreshCcw size={20} className="text-orange-500" /> 申请售后 /
              退款服务
            </h3>
            <button
              onClick={closeModal}
            ...
            <div className="bg-orange-50 border border-orange-100 rounded-xl p-5">
              ...
              <ul className="space-y-2 text-xs text-orange-700 list-disc pl-4">
                <li>
                  ...
                  按剩余天数折算，并收取剩余价值{" "}
                  <span className="font-bold">15%</span> 的违约金。
                </li>
                <li>
                  <span className="font-bold">资源包：</span>
                  未通过资源包全额退款；已使用（哪怕仅用1%）的资源包
                  <span className="font-bold">不予退款</span>。
                </li>
                <li>
                  <span className="font-bold">数据安全：</span>
                  退款申请通过后，云端存储的工艺数据将保留
                  <span className="font-bold">7天后永久擦除</span>。
                </li>
              </ul>
            </div>
                  ...
                  <div className="flex justify-between items-start pb-4 border-b border-gray-50">
                    ...
                    <div className="text-right">
                      <div className="text-xs text-gray-400 mb-1">
                        原价 ¥{selectedItem.amount.toLocaleString()}
                      </div>
                      <div className="text-xs text-red-500">
                        - ¥
                        {(selectedItem.amount * usageRatio).toLocaleString(
                          undefined,
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl p-6 relative">
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl transition-colors shadow-lg shadow-blue-200 flex items-center justify-center gap-2"
          >
            {isLoading ? (
