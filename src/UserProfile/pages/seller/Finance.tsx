...
import {
  SELLER_STATS,
  SELLER_REVENUE_CHART_DATA,
  SELLER_TRANSACTIONS,
} from '../../constants/profile';
import { useSellerMonitoring } from '../../hooks/useSellerMonitoring';
import { useSellerRefunds } from '../../hooks/useSellerRefunds';
import type { SellerMonitoringItem, BuyerOrder } from '../../types/profile'; 

const SellerFinance: React.FC = () => {
  const { globalOrders, openModal, onUpdateOrder } = useOutletContext<any>();
  const { monitoringData, updateMonitoring } = useSellerMonitoring();
  const { sellerRefunds, removeRefund } = useSellerRefunds();
  const [sellerFinanceSubTab, setSellerFinanceSubTab] = useState<'overview' | 'monitoring' | 'transactions'>('overview');

  const maxRev = Math.max(...SELLER_REVENUE_CHART_DATA.map((d) => d.value)) * 1.1;
  const underReviewItems = globalOrders.filter((item: BuyerOrder) => item.paymentStatus === 'UnderReview');
  const pendingInvoices = globalOrders.filter((item: BuyerOrder) => item.invoiceStatus === 'Pending');
                      ...
                      underReviewItems.map((item) => (
                        <div key={item.id} className="bg-gray-50 rounded-lg p-3 border border-gray-100 text-sm">
                          <div className="flex justify-between mb-1">
                            <span className="font-bold text-gray-900">{item.provider || '买家'}</span>
                          ...
                          <button
                            onClick={() => openModal('confirm_payment', item)}
                    ...
                    {pendingInvoices.length === 0 ? (
                      ...
                      pendingInvoices.map((item) => (
                        <div key={item.id} className="bg-gray-50 rounded-lg p-3 border border-gray-100 text-sm">
                          ...
                          <div className="text-xs text-gray-500 mb-3">{item.id}</div>
                          <button
                            onClick={() => openModal('issue_invoice', item)}
                    ...
                    {pendingRefunds.length === 0 ? (
                      ...
                      pendingRefunds.map((refund) => (
                        <div key={refund.id} className="bg-gray-50 rounded-lg p-3 border border-gray-100 text-sm">
                          ...
                          <button
                            onClick={() => openModal('seller_refund_audit', refund)}
                  ...
                  <tr key={item.id} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-right sticky right-0 bg-white group-hover:bg-gray-50/50 transition-colors z-10">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() => openModal('monitoring_detail', item)}
                        ...
                        {item.plan === '按量计费' ? (
                          item.status === 'unbilled' ? (
                            <button
                              onClick={() => openModal('generate_invoice', item)}
