...
export const PaymentStatusBadge = ({ status }: { status: string }) => {
  switch (status) {
    case 'PendingPayment':
      return (
        <span className="bg-yellow-50 text-yellow-600 border border-yellow-200 text-xs px-2 py-1 rounded w-fit font-medium">
          待支付
        </span>
      )
    case 'UnderReview':
      return (
        <span className="bg-blue-50 text-blue-600 border border-blue-200 text-xs px-2 py-1 rounded w-fit font-medium">
          审核中
        </span>
      )
    case 'Rejected':
      return (
        <span className="bg-red-50 text-red-600 border border-red-200 text-xs px-2 py-1 rounded w-fit font-medium">
          已驳回
        </span>
      )
    case 'Paid':
    ...
    case 'Cancelled':
      return (
        <span className="bg-gray-50 text-gray-600 border border-gray-200 text-xs px-2 py-1 rounded w-fit font-medium">
          已取消
        </span>
      )
    default:
...
export const StatusBadge = ({ status, expireDate }: { status: string; expireDate?: string }) => {
  ...
  if (status === 'Trial') {
    const daysLeft = expireDate
      ? Math.ceil((new Date(expireDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24))
      : 0
    return (
