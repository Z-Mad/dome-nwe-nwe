...
interface SellerRefundAuditModalProps {
  refund: SellerRefund;
  onClose: () => void;
  onAudit: (approved: boolean, comment: string) => void;
}

export const SellerRefundAuditModal: React.FC<SellerRefundAuditModalProps> = ({ refund, onClose, onAudit }) => {
      ...
      <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl p-6 relative">
        ...
        <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 mb-6">
          ...
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-2">
              <span className="font-bold text-gray-900">{refund.orderId}</span>
              <span className="text-[10px] bg-blue-50 text-blue-600 px-1.5 rounded border border-blue-100">企业版订阅</span>
            </div>
            <span className="font-bold text-red-600 text-lg">¥ {refund.amount}</span>
          </div>
          <div className="grid grid-cols-2 gap-4 text-xs">
            <div>
              <span className="text-gray-400 block mb-1">买家账户</span>
              <span className="text-gray-800 font-medium">{refund.buyer}</span>
            </div>
            <div>
              <span className="text-gray-400 block mb-1">申请时间</span>
              <span className="text-gray-800 font-medium">{refund.date}</span>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="text-sm font-bold text-gray-900">使用情况评估 (Usage Assessment)</span>
          </div>
          <div className="space-y-3 bg-white border border-gray-100 p-3 rounded-xl shadow-sm">
            <div>
              <div className="flex justify-between text-xs mb-1">
                ...
                <span className="font-bold text-gray-900">2天 / 30天 (6.7%)</span>
              </div>
            ...
            <div>
              <div className="flex justify-between text-xs mb-1">
                ...
                <span className="font-bold text-gray-900">1,240 / 10,000 (12.4%)</span>
              </div>
              <div className="w-full bg-gray-100 h-1.5 rounded-full overflow-hidden">
          ...
          <div className="mt-2 bg-orange-50 border border-orange-100 p-3 rounded-lg flex items-start gap-2">
            ...
            <p className="text-xs text-orange-700 leading-relaxed">
              提示：买家在使用期间产生过 3 次 4xx 错误调用，可能遇到兼容性问题。建议参考买家描述。
            </p>
          </div>
        </div>

        <div className="mb-4">
          <label className="block text-sm font-bold text-gray-700 mb-2">买家申请理由</label>
        ...
        <div className="mb-6">
          <label className="block text-sm font-bold text-gray-700 mb-2">审核意见 (必填/Optional)</label>
          <textarea
