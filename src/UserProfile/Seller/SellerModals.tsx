...
const getAssetModalData = (activeModal: string, params: any, sellerAssets: any[]) => {
  ...
  const version = params.versionId
    ? item?.versions?.find((v: any) => v.ver === params.versionId)
    : null
...
const getOrderOrMonitoringData = (
  activeModal: string,
  params: any,
  localOrders: any[],
  monitoringData: any[],
  ...
  if (activeModal === 'issue_invoice') {
    return {
      selectedItem: localOrders.find((o) => o.id === params.orderId) || { id: params.orderId },
  ...
  if (activeModal === 'generate_invoice' || activeModal === 'monitoring_detail') {
    return {
      selectedItem: monitoringData.find((m) => m.id === params.itemId) || { id: params.itemId },
      selectedVersion: null,
    }
  }
...
const getSelectedData = (
  ...
  if (activeModal === 'seller_refund_audit') {
    return {
      selectedItem: sellerRefunds.find((r) => r.id === params.refundId) || { id: params.refundId },
      selectedVersion: null,
    }
  }
