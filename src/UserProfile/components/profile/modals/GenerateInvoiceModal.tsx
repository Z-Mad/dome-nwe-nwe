...
export const GenerateInvoiceModal: React.FC<GenerateInvoiceModalProps> = ({ item, onClose, onGenerate, isLoading }) => {
      ...
      <div className="bg-white rounded-2xl max-w-lg w-full shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
        <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          <h3 className="text-xl font-bold text-gray-900 flex items-center gap-2">
            <FileText className="text-indigo-600" /> 生成请款单
          </h3>
        ...
        <div className="p-6 space-y-6">
          <div className="bg-indigo-50/50 rounded-xl p-4 border border-indigo-100">
            ...
            <div className="text-lg font-bold text-gray-900">{item.buyer}</div>
          </div>
          ...
          <div className="space-y-4">
            ...
            {item.billedPeriods && item.billedPeriods.length > 0 && (
              <div className="text-xs text-gray-500">
                <span className="text-red-500">*</span> 以下周期已生成请款单，不可重复选择：
                <ul className="list-disc pl-4 mt-1">
                  {item.billedPeriods.map((bp, i) => (
                    <li key={i}>{bp}</li>
                  ))}
                </ul>
              </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-gray-50 rounded-xl p-4 border border-gray-100 space-y-3 text-sm relative">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-medium text-gray-900">{item.asset} ({item.version})</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-500">订单编号</span>
                <span className="font-mono text-gray-900">{item.orderId}</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="pt-3 border-t border-gray-200 flex justify-between items-center">
                ...
                <span className="text-xl font-bold text-indigo-600 font-mono">¥ {item.estimatedCost.toFixed(2)}</span>
              </div>
            </div>
          </div>
        ...
        <div className="p-6 border-t border-gray-100 bg-gray-50 flex justify-end gap-3">
          ...
          <button
            onClick={() => onGenerate(invoiceStartDate, invoiceEndDate)}
