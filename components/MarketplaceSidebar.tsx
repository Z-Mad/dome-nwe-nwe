...
import {
  Search, LayoutGrid, Box, LineChart, Zap, Layers, User, Settings, LogOut, PlusCircle, Shield,
...
const MarketplaceSidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <div className="w-64 bg-white border-r border-gray-100 flex-shrink-0 flex flex-col h-full hidden md:flex sticky top-0 z-50">
      <div className="p-6 pb-2 flex items-center gap-3 cursor-pointer" onClick={() => onChangeView('discovery')}>
      ...
      <div className="flex-1 overflow-y-auto px-4 space-y-1 py-2 custom-scrollbar">
        {menuItems.map((item, idx) => (
          item.isHeader ? (
            <div key={idx} className="px-3 pt-6 pb-2 text-[10px] font-bold text-gray-400 uppercase tracking-wider">
              {item.label}
            </div>
          ) : (
            <button
              ...
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors relative group ${currentView === item.id || (currentView === 'detail' && item.id === 'method_agents')
      ...
      <div className="p-4 border-t border-gray-100 space-y-2">
        <button
          ...
          className={`w-full flex items-center gap-2 px-3 py-2 text-sm border rounded-lg justify-center transition-colors ${currentView === 'documentation' ? 'bg-blue-50 border-blue-200 text-blue-700 font-bold' : 'text-gray-500 hover:text-gray-700 border-gray-200 hover:bg-gray-50'
            }`}
        >
          <Book size={16} />
          <span>产品文档</span>
