...
export const useVirtualPagination = <T>(items: T[], options: UseVirtualPaginationOptions) => {
  ...
  const loadedItems = useMemo(() => items.slice(0, loadedCount), [items, loadedCount])
  ...
  const bottomSpacerHeight = Math.max(0, (loadedItems.length - visibleEnd) * itemHeight)
