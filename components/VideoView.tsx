          ...
          <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
            ...
            <div className="space-y-3">
              {videos.map((video) => (
                <div
                  key={video.id}
                  className={`flex gap-3 p-2 rounded-lg cursor-pointer transition-colors ${video.active ? 'bg-blue-50 border border-blue-100' : 'hover:bg-gray-50'}`}
                >
                  <div className="relative w-24 h-16 bg-gray-200 rounded overflow-hidden flex-shrink-0">
                    <img
                      src={`https://picsum.photos/seed/${video.id + 20}/200/120`}
                      alt="thumb"
                  ...
                  <div className="flex-1 min-w-0">
                    <h4
                      className={`text-sm font-medium leading-tight mb-1 ${video.active ? 'text-blue-600' : 'text-gray-800'}`}
                    >
                      {video.title}
                    </h4>
