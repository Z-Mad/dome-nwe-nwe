      ...
      <div className="max-w-5xl mx-auto">
         ...
         <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            <div>
                ...
                <p className="text-gray-500 mt-2">连接钢企真实痛点与开发者解决方案，总需求金额超 <span className="text-blue-600 font-bold">¥1.2亿</span></p>
            </div>
         ...
         <div className="space-y-4">
             {demands.map(item => (
                 <div key={item.id} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all group cursor-pointer relative overflow-hidden">
                     {item.status === 'urgent' && (
                         <div className="absolute top-0 right-0 bg-red-500 text-white text-xs px-3 py-1 rounded-bl-xl font-bold">急需</div>
                     )}
                     ...
                     <div className="flex flex-col md:flex-row gap-6">
                         <div className="flex-1">
                             ...
                             <div className="flex flex-wrap gap-2">
                                 {item.tags.map(tag => (
                                     <span key={tag} className="text-xs text-blue-600 bg-blue-50 px-2 py-1 rounded border border-blue-100 flex items-center gap-1">
                                         <Tag size={10}/> {tag}
                                     </span>
                                 ))}
                             </div>
