...
import type { SellerMonitoringItem, SellerRefund, BuyerOrder } from '../../types/profile';

interface SellerFinanceProps {
  monitoringData: SellerMonitoringItem[];
  orders: BuyerOrder[];
  refunds: SellerRefund[];
  onOpenModal: (type: string, item: any) => void;
  onUpdateMonitoring: (id: string, updates: Partial<SellerMonitoringItem>) => void;
  onUpdateOrder: (id: string, updates: Partial<BuyerOrder>) => void;
}

export const SellerFinance: React.FC<SellerFinanceProps> = ({
  monitoringData,
  orders,
  refunds,
  onOpenModal,
  ...
  const underReviewItems = orders.filter((item) => item.paymentStatus === 'UnderReview');
  const pendingInvoices = orders.filter((item) => item.invoiceStatus === 'Pending');
                    ...
                    {underReviewItems.length === 0 ? (
                      ...
                      underReviewItems.map((item) => (
                        <div key={item.id} className="bg-gray-50 rounded-lg p-3 border border-gray-100 text-sm">
                          ...
                          <button
                            onClick={() => onOpenModal('confirm_payment', item)}
                    ...
                    {pendingInvoices.length === 0 ? (
                      ...
                      pendingInvoices.map((item) => (
                        <div key={item.id} className="bg-gray-50 rounded-lg p-3 border border-gray-100 text-sm">
                          ...
                          <div className="text-xs text-gray-500 mb-3">{item.id}</div>
                          <button
                            onClick={() => onOpenModal('issue_invoice', item)}
                    ...
                    {pendingRefunds.length === 0 ? (
                      ...
                      pendingRefunds.map((refund) => (
                        <div key={refund.id} className="bg-gray-50 rounded-lg p-3 border border-gray-100 text-sm">
                          ...
                          <button
                            onClick={() => onOpenModal('seller_refund_audit', refund)}
                  ...
                  <tr key={item.id} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-right sticky right-0 bg-white group-hover:bg-gray-50/50 transition-colors z-10">
                      <div className="flex items-center justify-end gap-2">
                        <button
                          onClick={() => onOpenModal('monitoring_detail', item)}
                        ...
                        {item.plan === '按量计费' ? (
                          item.status === 'unbilled' ? (
                            <button
                              onClick={() => onOpenModal('generate_invoice', item)}
